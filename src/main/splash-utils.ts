...
export default class {
	...
	public static load(browserWindow: Electron.BrowserWindow): Promise<void> {
		browserWindow.loadFile(join(__dirname, '../renderer/html/splash.html'));
		...
		return new Promise(resolve => {
			browserWindow.webContents.once('did-finish-load', async() => {
				...
				await this.emitReleaseData(browserWindow);
				browserWindow.show();
				...
				ipcMain.once(MESSAGE_SPLASH_DONE, () => {
					...
					WindowUtils.destroyWindow(browserWindow);
	...
	private static async getReleaseData(): Promise<ReleaseData> {
		...
		if (!IS_DEVELOPMENT) {
			const response = await fetch(`https://api.github.com/repos/${ CLIENT_REPO }/releases/latest`);
